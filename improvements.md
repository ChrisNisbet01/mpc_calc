...
### 1. More Granular Error Handling (Location Information)
...
### 2. Abstract Syntax Tree (AST) Decoupling
...
*   **Problem:** The `tag_handlers` and `contents_handlers` arrays are static. Adding new functions or operations requires modifying and recompiling `formula_parser.c`.
*   **Suggestion:** Implement a dynamic registration mechanism for handlers. This could involve a function like `formula_register_handler(const char *tag, EvalResult (*handler_func)(mpc_ast_t const *tree, double x_value))`. This would allow users of the library (or future extensions) to add custom functions or constants at runtime without modifying the core library code.
...
*   **Suggestion:** Extend the grammar and add handlers for:
    ...
    *   **Functions:** `abs()`, `round()`, `ceil()`, `floor()`, `sqrt()`, `exp()`, `min()`, `max()`, `sign()`, `fact()` (factorial).
